    ...
    return (
        ...
        <div className="container-app">
            <div className="flex justify-between items-center py-4">
                {/* LOGO */}
                <Link to='/' className="flex gap-2 items-center text-xl text-primary-500 font-bold">
            ...
            <nav className="hidden md:flex space-x-3">
                {navLink.map( (link) => (
                    <Link key={link.path} to={link.path} className={
                        pathname === link.path ? 'text-primary-500 bg-primary-500/10 rounded-md h-10 px-3 py-2' : 'text-gray-400 h-10 py-2 px-3 hover:text-primary-500 hover:bg-primary-500/5 rounded-md'
                    }>
                     {link.name}
                    </Link>
                ))}
                ...
                <div>
                    {isAuthenticated ? (
                        ...
                        <nav className="space-y-1">
                            {navLink.map((link) => (
                            <Link to={link.path} key={link.path} className={`block p-5 rounded-lg ${pathname === link.path ? 'bg-gray-800 text-primary-500 font-medium' : 'text-gray-400 hover:bg-gray-800 hover:text-primary-500'}`}
